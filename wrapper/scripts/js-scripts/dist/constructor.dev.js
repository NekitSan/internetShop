...
$(document).ready(function () {
  ...
  $(".constructor__menu").children().on("click", function (e) {
    ...
    var volum = volume.replace(/_/g, '');
    var coat = dataBox.attr("data-coating");
...
button_param.click(function (e) {
  ...
  switch (id_param) {
    case "height":
      ...
      if (meaning == "680") {
        $("._680").removeClass("constructor__menu-disable"); // null values

        nullValues(meaning); // #null values
      ...
      if (meaning == "1000") {
        $("._1000").removeClass("constructor__menu-disable"); // null values
    ...
    case "diameter":
      ...
      class_click = class_click.replace(/_/g, ''); //console.log(class_click);
...
function () {
  ...
  _createClass(EditAquarium, [{
    ...
    key: "priceTrue",
    value: function priceTrue(coat, volum) {
      var priceAll = {
        90: {
          'Б': {
            'matt': 20500,
      ...
      var price = valuePrice(priceAll, volum, this.equipm, coat);
...
function valuePrice(priceAll, volum, quipmt, coat) {
  ...
  var lengthVolum = Object.keys(priceAll[volum]).length; // количество значений комплектаций

  var lengthQuipmt = Object.keys(priceAll[volum][quipmt]).length;

  for (var i = 0; i < lengthVolum; i++) {
    // Нужная комплектация
    var value = Object.keys(priceAll[volum])[i];

    if (value == quipmt) {
      // Нужное покрытие
      var trueQuipmt = value;

      for (var i = 0; i < lengthQuipmt; i++) {
        var trueCoat = Object.keys(priceAll[volum][trueQuipmt])[i];

        if (trueCoat == coat) {
          var _trueCoat = Object.keys(priceAll[volum][trueQuipmt])[i]; // Нужное цена

          var truePrice = priceAll[volum][trueQuipmt][_trueCoat];
          return truePrice;
        }
      }
    }
